...
改动 XLSX 版式时必须守住两条约束，否则字号会被整体改掉：

- **A..L 列宽总和不能变。** 表单设了 `fitToWidth=1`，列宽总和（当前 86.48333
  字符）略超 A4 可打印宽度，Excel 因此把整页缩到约 91.6%，`pdf_layout.py` 里
  每个 `size` 都是这个比例缩出来的。要给某一列加宽，只能从别的列腾挪，保持总和
  不变。制版后 diff 一遍 `pdf_layout.py`，`size` 字段全部不变才算没破坏字体。
- **行高可以加。** 内容总高约 678pt，缩放后仅占可打印高度的八成，高度不是缩放的
  约束边，加行高不会触发重新缩放，只会让下方内容整体下移。

第 8 行把 Name / Surname / Position 三组标签加数据区挤在一行，可腾挪的余量就是各
标签块末尾的留白。职位数据区 `K8:L9` 已按三行高度制版并设为底部对齐，PDF 叠加层
用 `document_generator.WRAPPED_FIELDS` 声明同样的行数——改模板行高要同步改它。
